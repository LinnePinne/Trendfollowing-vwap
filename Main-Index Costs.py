...
markets = [
    {
        ...
        "point_size": 1.0,              # 1 point = 1.0 i pris (ofta så för index CFD)
        "fixed_spread_points": 2.5,     # spread i index-points NAS = 1.2 DAX = 1.5
        "slippage_points": 1.5,         # slippage i index-points (per fill) NAD = 0.4 DAX = 0.5
        "comm_points_roundturn": 0.0,   # om ni modellerar commission som points per roundturn
    },
]
...
def run_backtest_for_market(market_name: str, csv_path: str, point_size: float, fixed_spread_points: float,
                            slippage_points: float, comm_points_roundturn: float):
    global close_price
    print("\n" + "=" * 70)
    ...
    def points_to_price(points: float, point_size: float) -> float:
        return float(points) * float(point_size)

    def commission_round_turn_price(comm_points_roundturn: float, point_size: float) -> float:
        # commission uttryckt i "points" -> pris
        return points_to_price(comm_points_roundturn, point_size)
    ...
    def get_spread_points(row, fixed_spread_points: float) -> float:
    ...
    for i in range(1, len(df) - 1):
        ...
        if in_position:
            ...
            if pos_direction == "LONG":
                if (prev_ema_fast > prev_ema_slow) and (ema_fast < ema_slow):
                    ...
                    spread_points = get_spread_points(next_row, fixed_spread_points)
                    spread_px = points_to_price(spread_points, point_size)
                    slip_px = points_to_price(slippage_points, point_size)
            ...
            if exit_price is not None:
                ...
                if pos_direction == "LONG":
                    pnl =  (exit_price - entry_price) / point_size   # LONG
                else:
                    pnl = (entry_price - exit_price) / point_size   # SHORT

                pnl -= comm_points_roundturn
        ...
        if short_signal and trend_filter:
            pos_direction = "SHORT"
            entry_time = idx_list[i + 1]
...
for m in markets:
    try:
        stats, trades_df = run_backtest_for_market(
            ...
            fixed_spread_points=m["fixed_spread_points"],
            slippage_points=m["slippage_points"],
            comm_points_roundturn=m["comm_points_roundturn"],
        )
